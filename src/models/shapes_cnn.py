...
#The models
def get_shapesModel(inputDim, outputDim, depth=1, hidden=4960, useDp=False, modelName='best_model'):
    ...
    if useDp:
        ...
        x = tf.keras.layers.Conv2D(filters=16,kernel_size=(7,7),strides=(3,3), padding="same", activation='relu')(x)
    else:
       x = tf.keras.layers.Conv2D(filters=16,kernel_size=(7,7),strides=(3,3), padding="same", activation='relu')(inp)
    x = tf.keras.layers.MaxPooling2D(pool_size=(3,3),strides=(1,1))(x)
    if depth > 1:
